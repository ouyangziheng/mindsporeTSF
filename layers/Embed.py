...
class PositionalEmbedding(nn.Cell):
    def __init__(self, d_model, max_len=5000):
        ...
        position = ops.arange(0, max_len).view(-1, 1).astype(mindspore.float32)
        div_term = ops.Exp()(ops.arange(0, d_model, 2).astype(mindspore.float32) * -(math.log(10000.0) / d_model))
    ...
    def init_parameters(self):
        for _, m in self.cells_and_names():
            if isinstance(m, nn.Conv1d):
                m.weight.set_data(Tensor(np.random.normal(0, math.sqrt(2. / m.in_channels), m.weight.shape).astype(np.float32)))
...
class FixedEmbedding(nn.Cell):
    def __init__(self, c_in, d_model):
        ...
        position = ops.arange(0, c_in).view(-1, 1).astype(mindspore.float32)
        div_term = ops.Exp()(ops.arange(0, d_model, 2).astype(mindspore.float32) * -(math.log(10000.0) / d_model))

        w = ops.Zeros()((c_in, d_model), mindspore.float32)
        w[:, 0::2] = ops.Sin()(position * div_term)
        w[:, 1::2] = ops.Cos()(position * div_term)

        self.emb = nn.Embedding(c_in, d_model, embedding_table=w, use_one_hot=False)
